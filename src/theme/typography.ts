 ...
 *
 * Each step is given a mobile value and a larger desktop one through `@media`. No web font is loaded: the stack resolves to Roboto on Android,
 * Segoe UI on Windows and the system face elsewhere, which costs nothing to download and cannot flash on first paint.
 */
...
/** Fonts present on every platform an archive is read on, in preference order. */
...
/**
 * The kit's type scale, mapped onto the variants an archive's pages actually ask for.
 *
 * The variants are not used the way MUI intends them: `h3` is the subject's name, while `h5` and `h6` are page and card titles. A theme cannot
 * rename a call site, so the sizes are hung on the variants as they are used rather than on the names MUI gives them. `h1` and `h2` stay defined
 * and unused, so a future page has something larger to reach for.
 */
export const archiveTypography: ThemeOptions["typography"] = {
	fontFamily: FONT_STACK,
	h1: step(40, 48, 700, { lineHeight: 1.05, letterSpacing: "-0.015em" }),
	h2: step(34, 40, 700, { lineHeight: 1.08, letterSpacing: "-0.01em" }),
	// The subject's name, the single largest thing an archive renders.
